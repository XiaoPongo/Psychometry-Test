<script lang="ts">
  ...
  // --- Types ---
  type Color = 'R' | 'Y' | 'G';
  type Phase = 'idle' | 'armed' | 'awaiting_W' | 'stimulus_on' | 'finished';
  ...
  type Trial = {
    ...
    premature: boolean;     // pressed R/Y/G while default before this stimulus
  ...
  const W_HOLD_MS = 200;
  const MAX_RT_MS = 2000;        // 2 seconds per input
  ...
  let wIsDown = false;
  let wDownAt = 0;
  let wGateTimer: number | null = null;
  ...
  // --- Theme: set LIGHT by default if not saved ---
  onMount(() => {
    const root = document.documentElement;
    const saved = localStorage.getItem(THEME_KEY);
    const desired = saved ?? 'light';
  ...
  onMount(() => {
    ...
    return () => {
      ...
      clearTimer();
      if (wGateTimer) clearTimeout(wGateTimer);
    };
  });
  ...
  function scheduleStimulusAfterWGate() {
    if (phase !== 'awaiting_W') return;
    if (wGateTimer) clearTimeout(wGateTimer);
    const now = performance.now();
    const alreadyHeld = now - wDownAt;
    const waitMs = Math.max(1, W_HOLD_MS - alreadyHeld);
    wGateTimer = window.setTimeout(() => {
      if (wIsDown && phase === 'awaiting_W' && performance.now() < testEndsAt) {
        showNextStimulus();
      }
  ...
  function handleKeyDown(e: KeyboardEvent) {
    ...
    if (key === 'W') {
      if (!wIsDown) {
        wIsDown = true;
        wDownAt = now;
        if (phase === 'awaiting_W' && now < testEndsAt) {
          scheduleStimulusAfterWGate();
        }
      }
      return;
    }
    ...
    if (phase === 'stimulus_on' && currentTrial) {
      ...
      if (isLetter) {
        ...
        if (!currentTrial.respondedAt) {
          ...
          if (!currentTrial.correct) currentTrial.wrongKey = key;
          phase = 'awaiting_W';
        }
      }
    }
  ...
  function handleKeyUp(e: KeyboardEvent) {
    ...
    if (key === 'W') {
      if (wGateTimer) {
        const held = now - wDownAt;
        if (held < W_HOLD_MS) {
          clearTimeout(wGateTimer);
          wGateTimer = null;
        }
      }
      wIsDown = false;
  ...
  function refreshTest() {
    ...
    wIsDown = false;
    wDownAt = 0;
    if (wGateTimer) { clearTimeout(wGateTimer); wGateTimer = null; }
    showSummary = false;
    summary = null;
  }

  // --- Formatters / labels ---
  function fmtMs(ms?: number) { if (ms == null || isNaN(ms)) return '—'; return `${Math.round(ms)} ms`; }
  function fmtPct(p: number) { if (isNaN(p)) return '—'; return `${(Math.round(p * 10) / 10).toFixed(1)}%`; }
  function mmss(ms: number) { const s = Math.ceil(ms / 1000); const m = Math.floor(s / 60); const sec = s % 60; return `${m}:${sec.toString().padStart(2, '0')}`; }
...
<style>
  :root { --bg:#0b0b0e; --panel:#15161a; --muted:#8a8f98; --good:#2ecc71; --warn:#f1c40f; --bad:#e74c3c; --text:#e8eaed; --accent:#1d4ed8; --border:#2a2d34; --pad-off:#ffffff; }
  :root[data-theme='light'] { --bg:#f7f8fb; --panel:#ffffff; --muted:#5b6270; --good:#1f8f4a; --warn:#b8860b; --bad:#c0392b; --text:#0f172a; --accent:#1d4ed8; --border:#e5e7eb; --pad-off:#ffffff; }
  ...
  /* Single Light Pad - default WHITE */
  .lightpad { width:200px; height:200px; border-radius: 18px; border:1px solid var(--border); background: var(--pad-off); color:#111; display:grid; place-items:center; margin: 0 auto; }
  ...
  .lightpad.on.Y { background:#f3c743; box-shadow: 0 0 28px #ffdb4d; color:#111; }
...
<div class="wrap">
  ...
  <div class="arena">
    <!-- Single light (default WHITE) -->
    <div class="lightpad {phase==='stimulus_on' ? 'on ' + (currentTrial?.color || '') : ''}" aria-live="polite" aria-label="Stimulus light">
      <div class="label">{phase==='stimulus_on' ? colorLabel(currentTrial?.color) : 'PRESS W KEY'}</div>
    </div>
    ...
    <div class="instructions">
      ...
      <ul>
        <li><strong>Start</strong> the test, then hold <span class="kbd">W</span> for at least <strong>200 ms</strong> to reveal a color.</li>
        <li>Respond with the matching key: <span class="kbd">R</span> = RED, <span class="kbd">Y</span> = YELLOW, <span class="kbd">G</span> = GREEN.</li>
        <li>After every response, hold <span class="kbd">W</span> again (≥200 ms) to show the next color. Until then, the pad stays <strong>WHITE</strong> and shows <strong>PRESS W KEY</strong>.</li>
        <li>Pressing any other letter while a color is shown is an <strong>error</strong>. Premature presses (R/Y/G while default white) are also errors.</li>
        <li>Max reaction window is <strong>{MAX_RT_MS} ms</strong>. If you don’t respond in time, it counts as a miss and returns to default white.</li>
...
{#if !hasFocus}
  <div class="overlay" on:click={() => window.focus()}>
    <div>
      ...
      This test uses keys: <span class="kbd">W</span> (gate), <span class="kbd">R</span>, <span class="kbd">Y</span>, <span class="kbd">G</span>.
    </div>
  </div>
{/if}
